...
vec3 get_bar_color(float bar_pos) {
    if (u_color_count <= 1) {
        return u_colors[0];
    }
    float t = bar_pos * float(u_color_count - 1);
    int idx = int(floor(t));
    float fract_t = t - float(idx);
...
void main() {
    ...
    if (u_mirror == 1) {
        y = abs(y - 0.5) * 2.0;
        if (y < bar_height) {
            float bar_pos = float(bar_index) / float(u_bar_count);
            vec3 color = get_bar_color(bar_pos);
            float intensity = 1.0 - (y / bar_height) * 0.3;
        ...
        if (y < bar_height) {
            float bar_pos = float(bar_index) / float(u_bar_count);
            vec3 color = get_bar_color(bar_pos);
            float intensity = 0.7 + 0.3 * (y / bar_height);
            fragColor = vec4(color * intensity, 1.0);
        } else {
            fragColor = vec4(0.0, 0.0, 0.0, 0.0);
        }
