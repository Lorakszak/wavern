...
- **Multi-layer compositing** - stack up to 7 visualization layers, each with independent type, parameters, colors, blend mode, and opacity; reorder, toggle visibility, and edit per-layer from the sidebar. Blend modes: **Normal** (standard alpha compositing), **Additive** (brightens by adding layer colors together), **Screen** (lightens without overexposing, like projecting two images onto the same screen), **Multiply** (darkens by multiplying colors, useful for shadow and texture overlays)
